...
const STATUS_ITEMS: { id: TransactionStatus; label: string }[] = [
...
export default function TransactionsClient() {
  ...
  const availableMonths = useMemo(() => {
    ...
    transactions.forEach((t) => {
      const date = moment(t.date).locale("fa");

      if (date.jYear() === tempYear) {
        months.add(date.jMonth() + 1);
      }
    });

    return JALALI_MONTHS.filter((month) => months.has(month.id));
  }, [transactions, tempYear]);
  // --------------------------------
  ...
  useEffect(() => {
    ...
    setTransactions(
      transactions.map((t) => ({
        ...
        paymentMethod: t.paymentMethod.toLowerCase() as
          | "cash"
          | "card"
          | "online",
      })),
                      ...
                      <div className="space-y-1.5">
                        ...
                        >
                          ...
                          {availableYears.length > 0 ? (
                            ...
                            <option disabled>هیچ تراکنشی یافت نشد</option>
                          )}
                        </select>
                      </div>
                      ...
                      <div className="space-y-1.5">
                        ...
                        >
                          ...
                          {availableMonths.length > 0 ? (
                            ...
                            <option disabled>ابتدا سال را انتخاب کنید</option>
                          )}
                        </select>
                      </div>
                      {/* Action Btn */}
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div className="min-w-0 flex-1">
                          ...
                          <p className="text-muted-foreground text-xs">
                            #
                            {(currentPage - 1) * itemPerPage +
                              paginatedData.indexOf(transaction) +
                              1}
                          </p>
                        </div>
                      </div>
